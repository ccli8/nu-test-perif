...
static void test_serial_tx_async(void)
{   
    event_callback_t event_callback(test_serial_async_callback);
...
void test_serial_async_callback(int event)
{
...
static void test_spi_master_async(void)
{
    ...
    for (int i = 0; i < sizeof (spi_buf) / sizeof (spi_buf[0]); i ++) {
        spi_buf[i] = i;
    }
...
REPEAT:
    ...
    memset(spi_buf2, 0xFF, sizeof (spi_buf2));
    ...
    callback_event = 0;
    event_callback_t callback(test_spi_master_async_callback);
    if (spi_master.transfer(spi_buf, sizeof (spi_buf) / sizeof (spi_buf[0]), spi_buf2, sizeof (spi_buf2) / sizeof (spi_buf2[0]), callback, SPI_EVENT_ALL)) {
        printf("%s FAILED\n", __func__);
#if (! MYCONF_SPI_AUTOSS)
        cs = 1;
#endif
    ...
    if (callback_event & SPI_EVENT_COMPLETE) {
        ...
        if (memcmp(spi_buf + MYCONF_SPI_ECHO_PLUS, spi_buf2 + 2, sizeof (spi_buf) - sizeof (MYCONF_TRAN_UNIT_T) * MYCONF_SPI_ECHO_PLUS)) {
            printf("%s Round %d FAILED\n", __func__, n_round ++);
            while (1);
        }
        else {
...
static void test_i2c_master(void)
{
    ...
    // Fill in transmit buffer
    for (int i = 0; i < sizeof (i2c_buf) / sizeof (i2c_buf[0]); i ++) {
        i2c_buf[i] = i;
    }

REPEAT:
    i2c_master.write(MYCONF_I2C_ADDR, i2c_buf, sizeof (i2c_buf), 1);
    memset(i2c_buf2, 0xFF, sizeof (i2c_buf2));
    i2c_master.read(MYCONF_I2C_ADDR, i2c_buf2, sizeof (i2c_buf2), 1);
    
    i2c_master.write(MYCONF_I2C_ADDR, i2c_buf2, sizeof (i2c_buf2), 1);
    i2c_master.write(MYCONF_I2C_ADDR, i2c_buf2, sizeof (i2c_buf2), 1);
    i2c_master.write(MYCONF_I2C_ADDR, i2c_buf2, sizeof (i2c_buf2), 1);
    memset(i2c_buf2, 0xFF, sizeof (i2c_buf2));
    i2c_master.read(MYCONF_I2C_ADDR, i2c_buf2, sizeof (i2c_buf2), 1);
    
    i2c_master.write(MYCONF_I2C_ADDR, i2c_buf2, sizeof (i2c_buf2));
    memset(i2c_buf2, 0xFF, sizeof (i2c_buf2));
    i2c_master.read(MYCONF_I2C_ADDR, i2c_buf2, sizeof (i2c_buf2));
    ...
    char *buf_pos = i2c_buf;
    char *buf_end = i2c_buf + sizeof (i2c_buf) / sizeof (i2c_buf[0]);
    char *buf2_pos = i2c_buf2;
    int data;
    int data2;
...
REPEAT:
    ...
    callback_event = 0;
    event_callback_t callback(test_i2c_master_async_callback);
    if (i2c_master.transfer(MYCONF_I2C_ADDR, i2c_buf, sizeof (i2c_buf) / sizeof (i2c_buf[0]), i2c_buf2, sizeof (i2c_buf2) / sizeof (i2c_buf2[0]), callback, I2C_EVENT_ALL, 0)) {
        printf("%s FAILED\n", __func__);
        while (1);
    }
    ...
    if (callback_event & I2C_EVENT_TRANSFER_COMPLETE) {
        ...
        if (memcmp(i2c_buf + 1, i2c_buf2, sizeof (i2c_buf) - sizeof (char) * 1)) {
            printf("%s Round %d FAILED\n", __func__, n_round ++);
            while (1);
        }
        else {
